...
export default function CheckAdminTokenValid(req: express.Request, res: express.Response, next: express.NextFunction) {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return res.status(401).send(ErrorResponseWithMessage("Token bilgisi gelmedi"));
  }
  try {
    ...
    if (role !== "admin") {
      return res.status(401).send(ErrorResponseWithMessage("Bu işlem için yetkiniz yok."));
    }
    ...
    return res.status(401).send(ErrorResponseWithMessage("Token geçersiz, tekrar giriş yapınız"));
  }
}