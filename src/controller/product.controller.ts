...
export async function getAllProducts(
  ...
  try {
    ...
    if (!list || list.length === 0) {
      return res
        .status(404)
        .send(ErrorResponseWithMessage("Ürün Girişi yapılmamış"));
    }
...
export async function addProduct(req: express.Request, res: express.Response) {
  ...
  try {
    ...
    product.netPrice = calculateNetPrice(
      product.isDiscount,
      product.price,
...
export async function deleteProductWithId(
  req: express.Request,
  res: express.Response
) {
  const {
  ...
  if (!productId) {
    return res
      .status(400)
      .send(ErrorResponseWithMessage("Lütfen Ürünün Id bilgisini gönderiniz."));
  }
  try {
...
export async function updateProduct(
  ...
  if (!productId) {
    return res
      .status(400)
      .send(ErrorResponseWithMessage("Lütfen ürün ID bilgisini gönderin."));
  }
  ...
  try {
    ...
    const updatedProduct = await ProductSchemas.findByIdAndUpdate(
      productId,
      product,
      { new: true }
    );
...
export async function addToStock(req: express.Request, res: express.Response) {
  ...
  if (!productId) {
    return res
      .status(400)
      .send(ErrorResponseWithMessage("Lütfen ürün ID Bilgisini belirtiniz."));
  }
  ...
  try {
    ...
    const isStockExist = updatedObject.stockStatus?.find(
      (x) => x.color?.toString() === body.color && x.size === body.size
    );
    ...
    const response = await ProductSchemas.findByIdAndUpdate(
      productId || updatedObject._id,
      updatedObject,
      { new: true }
    );
...
export async function deleteFromStock(
  req: express.Request,
  res: express.Response
) {
  ...
  if (!stockId || !productId) {
    return res
      ...
      .send(
        ErrorResponseWithMessage(
          `Lütfen ürün ve stock Id bilgilerini geçiniz. stockId:${stockId}, productId: ${productId}`
        )
      );
  }
  ...
  try {
    ...
    if (!product) {
      return res
        .status(404)
        .send(ErrorResponseWithMessage("Ürün koduyla bulunamadı."));
    }
    const productObject: IProduct = product.toObject();
    productObject.stockStatus = productObject?.stockStatus?.filter(
      (x: any) => x._id.toString() !== stockId
    );

    const savedProduct = await ProductSchemas.findByIdAndUpdate(
      productId,
      productObject,
      { new: true }
    );
