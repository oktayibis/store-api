...
export async function addToCart(req: express.Request, res: express.Response) {
  ...
  try {
    ...
    const isDuplicated = mutatedUser?.carts?.find(x => x.productId?.toString() === body.productId && x.size === body.size);
    console.log("dup", isDuplicated);
    ...
    const updatedUser = await UserSchemas.findByIdAndUpdate(user.userId, mutatedUser, { new: true }).select("carts");
    return res.status(201).send(SuccessResponse(updatedUser));
...
export async function deleteFromCart(req: express.Request, res: express.Response) {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  const { body: { cartItemId }, user } = req;

  try {
    const foundUser = await UserSchemas.findById(user.userId);
    if (!foundUser) return res.status(400).send(ErrorResponseWithMessage("Kullanıcı bulunamadı."));

    const firstLength = foundUser.carts?.length;
    foundUser.carts = foundUser.carts?.filter(c => c._id?.toString() !== cartItemId);

    if (firstLength === foundUser.carts?.length) return res.status(403).send(ErrorResponseWithMessage("Sepette ürün bulunamadı."));

    const updatedCart = await UserSchemas.findByIdAndUpdate(user.userId, foundUser.toObject(), { new: true }).select("carts");
    return res.status(200).send(SuccessResponse(updatedCart));
  } catch (error: any) {
    return res.status(500).send(ErrorResponse(error));
...
export async function updateCart(req: express.Request, res: express.Response) {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  const { params: { cartItemId }, body, user } = req;
  ...
  try {
    ...
    const updatedCartUser = await UserSchemas.findByIdAndUpdate(user.userId, foundUser.toObject(), { new: true }).select("carts");
