...
export async function getUser(req: express.Request, res: express.Response) {
  try {
    ...
    if (!registeredUser) {
      return res
        .status(404)
        .send(ErrorResponseWithMessage("Kullanıcı bilgilerine ulaşılamadı."));
    }
...
export async function register(req: express.Request, res: express.Response) {
  ...
  try {
    ...
    if (isRegisteredBefore) {
      return res
        .status(406)
        .send(ErrorResponseWithMessage("Bu email daha zaten kayıtlıdır.", 406));
    }
...
export async function login(req: express.Request, res: express.Response) {
  ...
  try {
    ...
    const registeredUser = (await User.findOne({ email: body.email }).select(
      "+password"
    )) as any;
    if (!registeredUser) {
      return res
    ...
    const token = jwt.sign(
      { userId: registeredUser._id, role: registeredUser.role },
    ...
    if (response.acknowledged === false) {
      return res
        .status(500)
        .send(ErrorResponseWithMessage("Kullanıcı Güncellemesi başarısız."));
    }
    return res
      .status(200)
      .send(
        SuccessResponseWithCostumField(registeredUser.toObject(), { token })
      );
  } catch (error: any) {
    return res.status(500).send(ErrorResponse(error));
  }
...
export async function resetPasswordStep1(
  ...
  if (!email) {
    return res
      .status(400)
      .send(ErrorResponseWithMessage("Eksik Bilgi Gönderildi"));
  }
  ...
  if (!resetPassword) {
    ...
    if (!response.acknowledged) {
      return res
        .status(500)
        .send(ErrorResponseWithMessage("Şifre Sıfırlama Kodu Güncellenemedi."));
    }
  }
  return res
    .status(200)
    .send(SuccessResponse({ resetCode }, "Şifre sıfırlama kodu gönderildi."));
}
...
export async function resetPasswordStep2(
  ...
  try {
    ...
    if (!resetPassword)
      return res
        .status(404)
        .send(ErrorResponseWithMessage("Şifre Sıfırlama Kodu Bulunamadı."));

    // check resetCode
    if (resetPassword.resetCode !== resetCode)
      return res
        .status(404)
        .send(ErrorResponseWithMessage("Şifre Sıfırlama Kodu Hatalı."));
    ...
    if (now > expireDate)
      return res
        .status(500)
        .send(ErrorResponseWithMessage("Şifre Sıfırlama Kodu Geçerli Değil."));
    ...
    if (!response.acknowledged)
      return res
        .status(500)
        .send(ErrorResponseWithMessage("Şifre Güncellenemedi."));
    ...
    if (!response2)
      return res
        .status(500)
        .send(ErrorResponseWithMessage("Şifre Sıfırlama Kodu Silinemedi."));
